package com.x3.circus

import android.content.Context

/** Persisted settings (SharedPreferences). */
class Settings(ctx: Context) {
    private val prefs = ctx.getSharedPreferences("x3circus", Context.MODE_PRIVATE)
    ...
    /**
     * Gentler pacing: the formation moves slower and fires less often. Not a
     * difficulty ladder — the brief was a colourful game that is fun rather
     * than punishing, and this is the dial for people who mean it.
     */
    var relaxed: Boolean
        get() = prefs.getBoolean("relaxed", false)
        set(v) { prefs.edit().putBoolean("relaxed", v).apply() }
}
